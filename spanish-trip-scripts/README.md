# 🇪🇸 Spanish Trip Survival Scripts — Valencia 🍊

Practice scripts for a week-long trip to **Valencia**, built for **two people to practise together**.
Aimed at an **A1 / A2** learner who already knows a fair bit of vocabulary.

> **Spanish variety:** These use **Castilian Spanish (Spain)** — including *vosotros*
> and the *ceceo* "th" sound (e.g. *gracias* → "GRAH-thee-ahs", *cerveza* → "ther-BEH-tha").
> Castilian Spanish is spoken and understood everywhere in Valencia.

## 🍊 Valencia notes (read this!)

- **Language:** Valencia is bilingual — **Valencian (*valencià*)** is co-official with Spanish.
  You'll see it on street signs, menus and buildings (e.g. *Ajuntament* = town hall,
  *carrer* = street, *plaça* = square). **Everyone understands Spanish**, so you don't
  need Valencian — but a couple of courtesy words earn big smiles:
  - *Bon dia* — Good morning (Val.) `[bon DEE-ah]`
  - *Gràcies* — Thanks (Val.) `[GRAH-see-es]`
  - *Adéu* — Bye (Val.) `[ah-DEH-oo]`
- **Paella is FROM Valencia** — and locals are proud of it. Authentic *paella valenciana*
  has chicken, rabbit and beans (**not** seafood — that's *paella de marisco* or *mixta*).
  It's traditionally a **lunch** dish, not dinner. Ordering it well = instant respect. 😄
- **Horchata (*orxata*) + fartons** — chilled tiger-nut milk with sweet pastries for dipping.
  A classic Valencian café treat, especially in summer.
- **Agua de Valencia** — the city's famous cocktail (cava + orange juice + gin/vodka).
  Fun for the bar script — but strong, so pace yourselves!
- **Local dishes to try:** *fideuà* (paella made with noodles), *esgarraet* (roast pepper
  & cod salad), *all i pebre* (eel & potato stew), *coca* (savoury/sweet flatbread).

## How to practise (two people)

Each script has two roles:

- 🧳 **TURISTA** (Tourist) — that's you, the traveller.
- 🧑‍💼 **NATIVO/A** (Local: receptionist, waiter, passer-by…)

**Suggested drill:**
1. Person A is the Tourist, Person B is the Local. Read through once with translations.
2. Swap roles and do it again.
3. Cover the English column and try again.
4. Use the **🔁 Swap-in vocab** at the bottom of each script to change details
   (different food, room type, place names) so it doesn't get memorised word-for-word.

## Reading the pronunciation guide

Simple English-style phonetics are given in `[brackets]` under the Tourist's lines.

- Stress the syllable in **CAPS**.
- `th` = soft English "th" as in *think* (Castilian *c/z* sound).
- `rr` = rolled/trilled r. `r` = a light tap.
- `h` in the guide is silent-ish / very soft (Spanish *h* is silent; Spanish *j*/*g* is a raspy "kh").

## The scripts

| # | Scenario | File |
|---|----------|------|
| 1 | 🏨 Hotel — check-in, questions, problems, check-out | [`01-hotel.md`](01-hotel.md) |
| 2 | ☕ Café — coffee, breakfast, paying | [`02-cafe.md`](02-cafe.md) |
| 3 | 🍽️ Restaurant — seating, ordering, the bill | [`03-restaurant.md`](03-restaurant.md) |
| 4 | 🍷 Bar & tapas — drinks, small plates, casual chat | [`04-bar-tapas.md`](04-bar-tapas.md) |
| 5 | 🗺️ Directions — both asking **and** giving | [`05-directions.md`](05-directions.md) |
| 6 | 🚕 Taxi — telling the driver, the ride, paying | [`06-taxi.md`](06-taxi.md) |

## 🚀 Speed-run learning toolkit (read these too!)

The scripts get you through fixed situations. These pages get you **holding real
conversations** so you can keep learning from native speakers — which is the whole point.

| # | Page | What it's for |
|---|------|---------------|
| 7 | 🎣 [Keep the Conversation Alive](07-keep-the-conversation-alive.md) | **Most important page.** Fillers, lifelines & the anti-English-switch phrase |
| 8 | 🧱 [Lego Spanish — Sentence Builder](08-lego-spanish-sentence-builder.md) | ~12 frames → hundreds of sentences |
| 9 | 🎭 [Weird Memory Tricks & 5-Day Plan](09-weird-memory-tricks-and-5-day-plan.md) | Absurd mnemonics + a day-by-day study plan |
| 10 | 🗣️ [Free-Conversation Prompts](10-free-conversation-prompts.md) | Improv challenges for two (no scripts!) |
| 11 | 👂 [Listen Out For This](11-listen-out-for-this.md) | What locals will say to **you** + go-to replies |
| 12 | 🚌 [Bus](12-bus.md) | Catching a city bus — finding, paying, getting off |
| 13 | 🌍 [About Us — Where We're From](13-about-us-where-were-from.md) | Personalised "where are you from?" answers |
| 14 | 😄 [Humour & Charm](14-humour-and-charm.md) | Self-deprecating lines to disarm & win people over |
| 15 | 🎸 [Nightlife, Dives & Chatting to Locals](15-nightlife-dives-and-chatting-to-locals.md) | Dive-bar/gig know-how, talking to the pair of you, + safety |
| 16 | 🗣️ [What They'll Fire Back](16-what-theyll-fire-back.md) | Realistic replies you'll HEAR in every scenario + key words to catch |

## Universal survival phrases (learn these first!)

| English | Spanish | Say it like |
|---------|---------|-------------|
| Hello / Hi | Hola | OH-lah |
| Good morning | Buenos días | BWEH-nos DEE-as |
| Good afternoon/evening | Buenas tardes | BWEH-nas TAR-des |
| Good night | Buenas noches | BWEH-nas NOH-ches |
| Please | Por favor | por fah-BOR |
| Thank you (very much) | (Muchas) gracias | (MOO-chas) GRAH-thee-as |
| You're welcome | De nada | deh NAH-dah |
| Yes / No | Sí / No | see / noh |
| Excuse me (to get attention) | Perdone / Disculpe | per-DOH-neh / dees-KOOL-peh |
| Sorry | Lo siento | loh see-EN-toh |
| Do you speak English? | ¿Habla inglés? | AH-blah een-GLES |
| I don't understand | No entiendo | noh en-tee-EN-doh |
| Can you repeat that, please? | ¿Puede repetir, por favor? | PWEH-deh reh-peh-TEER por fah-BOR |
| More slowly, please | Más despacio, por favor | mas des-PAH-thee-oh por fah-BOR |
| I'm learning Spanish | Estoy aprendiendo español | es-TOY ah-pren-dee-EN-doh es-pan-YOL |
| How much is it? | ¿Cuánto es? / ¿Cuánto cuesta? | KWAN-toh es / KWAN-toh KWES-tah |
| Where is…? | ¿Dónde está…? | DON-deh es-TAH |
| The bill/check, please | La cuenta, por favor | lah KWEN-tah por fah-BOR |
| Goodbye | Adiós | ah-dee-OS |
| See you later | Hasta luego | AS-tah loo-EH-goh |

**Pro tip:** A friendly *"Hola, buenos días"* + *"por favor"* + *"gracias"* gets you a very
long way. Spaniards really appreciate the effort even if the rest is rocky. 😊
